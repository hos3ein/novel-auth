...
class LoginOptionalCodePass
{
    ...
    public function handle(Request $request, $next)
    {
        ...
        if (in_array(config(Constants::$configLoginOptions), [Constants::$OPTION_PASSWORD_CODE, Constants::$OPTION_CODE_PASSWORD])) {
            ...
            if ($force_otp_type) {
                ...
                if (config(Constants::$configLoginOptions) == Constants::$OPTION_PASSWORD_CODE) {
                    return RS::go2Password($request->claims, __('novel-auth::messages.login.pass'), $otpOptions, !empty($otpOptions));
                } else {
                    if (empty($otpOptions)) {
